...
# Current Focus
Cocktail Library + My Bar UX improvements (in progress, 2026-09-06 - audit + a 5-stage plan agreed, refined with 5 concrete requirements before Stage 1 started, see Decisions below). Folds in the Build Your Bar follow-up below (grouped Library headings) as part of the same effort. Stage 1 (`findRecipesUsingIngredient()` - pure domain matching for "which recipes use this ingredient," no UI yet) done and committed 2026-09-06. Stage 2 (Library's grouped availability view - Ready to Pour/Good Enough/Almost There/Unavailable, hide-if-empty, reusing `CocktailCard`) code-complete and tested 2026-09-06, being committed now so the user can sync it to their phone - mobile verification **pending**. Full audit + design in current-context.md.
...
# Backlog
1. Cocktail Library + My Bar UX improvements - Stages 1-2 done (see Current Focus). Stages 3-5 (ingredient/bottle detail screen + `?ingredient=` Library filter; My Bar tap-to-view wiring, kept separate from Build Your Bar's tap-to-select; final docs/regression) not started.
