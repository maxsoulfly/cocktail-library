...
# Current Focus
None chosen yet. Homepage "Build your bar" for an empty My Bar is **complete** as of 2026-09-06 - all 4 stages done, committed, and browser/mobile-verified on a real iPhone across every stage (Stage 4 reused the confirmed Stage 1-3 checks rather than re-testing, per the user's own instruction). Full design + real catalog data behind the six/expanded list in current-context.md. Pick the next focus from Next/Backlog below.
...
# Backlog
(empty - Homepage "Build your bar" shipped 2026-09-06; see Current Focus/Decisions and current-context.md for the full build record)
...
- **New, found 2026-09-06 while mobile-verifying Stage 3, not built (explicitly deferred to a later pass)**: the availability-sorted Library view (`?sort=availability`, `LibraryScreen.jsx`) reorders the flat grid correctly (perfect → good → almost → unavailable) but doesn't visually group results under headings - a member scrolling through it can't see where one tier ends and the next begins, just a change in badge color per card. Would need real headings (e.g. "Ready to Pour"/"Good Enough"/"Almost There"/"Unavailable", matching Home's own section names) inserted between tiers in the grid. Worth doing whenever Library/Home visuals are next touched.
