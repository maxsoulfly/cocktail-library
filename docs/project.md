...
# Current Focus
Homepage "Build your bar" for an empty My Bar (in progress, 2026-09-06 - audit + a 4-stage plan agreed, revised once per real UX corrections from the user, see Decisions below). Stage 1 (the essentials list itself: a pure domain resolver + `/bar?focus=1` autofocus, no homepage UI wiring yet) code-complete and tested 2026-09-06, awaiting review/commit. Full design + real catalog data behind the six/expanded list in current-context.md.
...
# Backlog
1. Homepage "Build your bar" - Stage 1 done (see Current Focus). Stages 2-4 (selection UI + stable session visibility together, availability-sorted "Show my cocktails", final docs/regression) not started.
...
- **Two things flagged 2026-09-06 for Build Your Bar Stage 2, recorded so they don't get lost before that stage starts**: (1) "Includes substitutions" must be real visible helper text under the live makeable-count, not a tooltip alone. (2) The per-visit "was the bar empty when I arrived" snapshot needs an explicit reset keyed on the signed-in user id, not just on the catalog/inventory finishing its load - as designed it could otherwise leak a previous account's "already saw it" state into a freshly-signed-in different account on the same device without a full page reload. Neither is built yet - Stage 1 has no homepage UI at all.
