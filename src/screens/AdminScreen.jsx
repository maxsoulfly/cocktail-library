...
export default function AdminScreen() {
  ...
  // Ingredient Types tab: same split-out reasoning as Classic Recipes above
  // - editing already existed (My Bar's admin pencil, now IngredientTypeEditor
  // shared with this tab), but delete had zero UI despite the "ingredient_types:
  // admin delete" RLS policy existing since the RLS-hardening pass. No pre-check
  // for in-use: a child type, product, recipe component, or substitution
  // alternative referencing this type all have their own restricting FK, so a
  // real delete attempt on an in-use type surfaces the DB's own error as-is,
  // same precedent as glasses/taste tags/families in the Catalog tab above.
  ...
  // Users tab: role changes and block/unblock, both admin-gated SECURITY
  // DEFINER functions - see 20260823100000_admin_user_management.sql for
  // why (profiles.role and memberships had no admin write path at all
  // before this). confirmAction holds { userId, type: "block" | "promote" }
  // - promoting to admin and blocking both get an inline confirm since they
  // grant or cut off real access; demoting and unblocking don't, matching
  // the app's existing pattern (delete/unpublish confirm, plain edits
  // don't). Neither action ever renders for the signed-in admin's own row -
  // the backend already refuses a self-target, but there's no reason to
    ...
    >
      ...
      <div style={{ padding: "20px" }}>
                ...
                <Card key={r.id} style={{ padding: "14px 16px" }}>
                  ...
                  {confirmDeleteClassicId === r.id && (
                    ...
                    >
                      ...
                      >
                        Delete "{r.name}"? This removes it from the catalog for
                        every member and can't be undone.
                      </p>
                      <div style={{ display: "flex", gap: 8 }}>
                        <Btn
                          variant="danger"
        ...
        {tab === "users" && (
          ...
          >
            <p style={{ margin: 0, fontSize: 13, color: "var(--text2)" }}>
              ...
              (RLS-enforced) without deleting their account or data; unblock
              reverses it. You can't change your own role or block yourself.
            </p>
            <Input
        ...
        {tab === "types" && (
          ...
          >
            <p style={{ margin: 0, fontSize: 13, color: "var(--text2)" }}>
              Every ingredient type in the catalog. Delete only succeeds if
