...
export default function AddProductScreen() {
  ...
  const matchedType = types.find((t) => t.name.toLowerCase() === ingType.toLowerCase())
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            {matchedType && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", background: "rgba(52,211,153,0.08)", borderRadius: 6, border: "1px solid rgba(52,211,153,0.2)" }}>
                ...
                <span style={{ fontSize: 13, color: "var(--green)" }}>Matches catalog ingredient: <strong>{matchedType.name}</strong></span>
              </div>
            )}
            {ingType && !matchedType && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", background: "rgba(251,191,36,0.08)", borderRadius: 6, border: "1px solid rgba(251,191,36,0.2)" }}>
                ...
                <span style={{ fontSize: 13, color: "var(--amber)" }}>Doesn't match an existing ingredient type - new types require admin approval, so this can't be added yet.</span>
              </div>
            )}
          </div>
          ...
          {matchedType && (
            <Card style={{ padding: "14px", border: "1px solid var(--border-s)", background: "var(--surface2)" }}>
              ...
              <div style={{ fontSize: 13, color: "var(--text2)", lineHeight: 1.5 }}>
                <strong style={{ color: "var(--text)" }}>{name || "Your Product"}</strong> will satisfy the <strong style={{ color: "var(--cyan)" }}>{matchedType.name}</strong> requirement in recipes.
              </div>
            </Card>
          )}
        ...
        <Btn variant="primary" full onClick={handleAdd} disabled={busy || !name || !matchedType}>Add to My Bar</Btn>
      </div>
    </div>
  )
}
