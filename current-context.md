...
**User-requested addition, same session**: `HomeScreen`'s Buy Next cards now have a "mark as owned" button (checkmark icon) that calls the same shared `inventory.toggleType()` used by My Bar - since it's the same optimistic-update state, marking an ingredient owned here immediately recalculates `computed`/`buyNext`, so the card naturally disappears from the list (no separate "owned" visual state needed on the card itself).
