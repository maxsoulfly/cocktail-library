...
- New migration clean on `db advisors --type all` (only the now-familiar accepted WARNs).
- New `recipes` embed for `fetchCommunityRecipes` verified valid via direct REST call (200, not 400).
- **DB logic verified directly** (see above) — both the happy path and the specific unauthorized-rejection scenario the null-safety fix addresses.
- **Not yet browser-verified**: the actual UI flow (clicking Publish/Unpublish, seeing the moderation tab populate) hasn't been clicked through yet.

## Remaining / not started

Step 12-13. Plus, carried over: no editor UI for substitution alternatives/hierarchy (step 8), no RLS/integration test harness for anything beyond what's been manually smoke-tested (step 8), real recipe *editing* UI (step 6), `<datalist>` theming (step 6, user-accepted). Admin Overview/Invitations/Batch Import tabs still mock (step 12).
