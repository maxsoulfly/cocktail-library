...
- **Second pass (still 2026-08-16) found two UI polish issues, both fixed:**
  1. The recipe editor's ingredient "amount" field was triggering Chrome's credit-card autofill suggestion — a short numeric text input with no `name`/`autocomplete`, sitting in a row of several short fields, matches Chrome's payment-form heuristic. Fixed with an explicit `name`, `autoComplete="off"`, and `inputMode="decimal"`.
  2. The unit/role `<select>` dropdowns in the same row showed up unthemed (native OS popup, not CSS-stylable — a real browser limitation, not a bug in the app's CSS). Rather than a partial `<option>` background-color hack (inconsistent cross-browser support), added a proper `Select` primitive (`src/components/primitives.jsx`) — a small custom dropdown matching the button-chip style already used elsewhere in the same screen (glass/family/taste-tag pickers) — and swapped both native selects for it. The ingredient-type/product-name `<datalist>` autocomplete inputs elsewhere are still native (no realistic CSS-only fix exists for those; a themed replacement would mean building a full custom combobox, not attempted here).
